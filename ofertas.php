...
require_once('Connections/cone.php');
	...
	if ($row_rs_moneda['moneda']==1) {
		$query_rs_pro = "SELECT * FROM volvo_productos WHERE oferta = '1' AND precio_oferta<>'0' AND habilitado = '1' $busca $orden";
		//$query_rs_pro = "SELECT * FROM volvo_productos WHERE oferta = '1' AND habilitado = '1' $busca $orden";
	}
	if ($row_rs_moneda['moneda']==2) {
		$query_rs_pro = "SELECT * FROM volvo_productos WHERE oferta = '1' AND precio_oferta_dolar<>'0' AND habilitado = '1' $busca $orden";
		///$query_rs_pro = "SELECT * FROM volvo_productos WHERE oferta = '1' AND habilitado = '1' $busca $orden";
	}
...
<div class="ruta">INICIO &gt; promociones</div>
...
?>Promociones</span>
	...
	if ($row_rs_moneda['moneda']==1) {
		...
		$query_rs_cuenta = "SELECT * FROM volvo_productos WHERE oferta = '1' AND subcategoria = '$subcategoria' AND precio_oferta<>'0' AND habilitado = '1'";
	}
	if ($row_rs_moneda['moneda']==2) {
		
		$query_rs_cuenta = "SELECT * FROM volvo_productos WHERE oferta = '1' AND subcategoria = '$subcategoria' AND precio_oferta_dolar<>'0' AND habilitado = '1'";
	}	
	...
	if (($totalRows_rs_cuenta<>0) && ($row_rs_b2['subcategoria']<>'')) {			
		?>
			  ...
			  <li><a href='ofertas.php?subcategoria=<?php echo $subcategoria; ?>&familia=<?php echo $familia;?>'><?php echo ucfirst(strtolower($row_rs_b2['subcategoria']));?> (<?php 	  
			  	echo $totalRows_rs_cuenta;
			  ?>)</a>
              <?php 
			  //if ($row_rs_b2['id']==$subcategoria['id']) {
